...
from util import read_text, open_keyb, open_keya, name_choise, message_RSA_text, get_prime_number_forword, numbers_to_string
...
class Window_2(QWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.text.setHtml(f"<font style='font-family: Arial; font-size: 12pt;'>{read_text('Theory_RSA.txt')}")
        layout.addRow(self.text)
        # Page Example
        page_example = QWidget(self)
        layout_ex = QFormLayout(self)
        page_example.setLayout(layout_ex)
        layout_ex.addRow(QLabel('Шифрование и расшифрование по алгоритму RSA'))
        ...
        self.e_B, self.d_B, self.e_A, self.d_A = RSA_word(self.p1, self.p2, self.q1, self.q2, self.open_key_A, self.open_key_B, self.m)
        print(self.e_B, self.d_B, self.e_A, self.d_A)
        ...
        self.inp_tsk_r.setFixedSize(620, 25)
    ...
    def click_btn_ex(self):
        try:
            ...
            v_exmpl_index = int(self.index)
            self.outp_ex.setHtml(f"<font style='font-family: Arial; font-size: 12pt;'>{RSA_Formula_Word(v_exmpl_p1, v_exmpl_p2, v_exmpl_q1, v_exmpl_q2, v_exmpl_oA, v_exmpl_oB, v_exmpl_m, v_exmpl_index)}")
    ...
    def click_btn_tsk_chk(self):
        try:
            ...
            if self.index == 0:
                v_r = numbers_to_string(e_B)
            elif self.index == 1:
                v_r = self.d_B
            elif self.index == 2:
                v_r = numbers_to_string(e_A)
            elif self.index == 3:
                v_r = self.d_A
